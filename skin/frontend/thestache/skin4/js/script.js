...
jQuery(window).load(function() {
  ...
  jQuery(window).scroll(function(){ headerSearch(); wideMenuPos(); contentTop()});
  ...
  /* Coin Slider */
	...
	function footerLinksAnimation() {
		if(jQuery('.footer-links').length){
			if (jQuery(document.body).width() > 767){
				...
				headerHeight = jQuery('header#sticky-header').outerHeight();
	...
	if(jQuery('.twitter-timeline').length){
		jQuery('.twitter-timeline').contents().find('head').append('<style>body{color: #888} body .p-author .profile .p-name{color: #222}</style>');
	}
	...
	function faqBlock(){
		if(jQuery('.home-sidebar .faq.block').length){
			blockTitle = jQuery('.home-sidebar .faq.block .block-title');
			itemsWrapper = jQuery('.home-sidebar .faq.block .items-wrapper');
			if(jQuery(document.body).width() > 767 && jQuery(document.body).width() < 1279) {
				jQuery('.home-sidebar .faq.block .items-wrapper').hide();
				if(blockTitle.find('i').length == 0) {
					blockTitle.children('h2').before('<i class="fa fa-plus"></i><i class="fa fa-minus"></i>');
				}
				blockTitle.removeClass('open');
				if(!blockTitle.hasClass('open')){
					blockTitle.off().on('click', function(){
						blockTitle.toggleClass('open');
						itemsWrapper.slideToggle();
					});
				}
			} else {
				blockTitle.removeClass('open');
	...
	contentTop();
	pageNotFound();
	jQuery(window).resize(function(){toDo(); replacingClass(); more_view_set_height(); wishlist_set_height(); related_set_height(); MCRemoveLink(); headerSearch(); footerLinksAnimation(); mobileSidebar(); headerCustomer2(); productTitlesHeight(); backgroundWrapper(); faqBlock(); wideMenuPos(); contentTop(); });
	/* Responsive */
